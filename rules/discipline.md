...
- **No workaround chains.** When something doesn't work as expected, investigate WHY and bridge the gap with one architectural mechanism. Do not add a parallel code path with flags to deduplicate — each workaround creates the next bug.
...
- **Copy the nearest similar example** as a template. An existing file inherits non-obvious conventions — naming, error handling, import order — that grep won't surface and descriptions won't capture.
...
- **Check all callers before changing a function.** Use Grep to find every call site. Three-quarters of agent maintenance iterations introduce regressions — most from signature changes that silently break callers outside the immediate context.
...
- **Match prompt verbs to plan verbs.** "Assess" in the plan = "assess and report, do not modify" in the prompt. Mismatched verbs are the primary cause of subagents modifying files during audits or doing read-only work during implementations.
